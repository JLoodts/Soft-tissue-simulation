...
int		maxNrWalls		= 100;
int		maxNrObjects	= 10;
double	minRadius		= 0.0007;	// minimum radius of the balls
double	maxRadius		= 0.0008;// radius of the balls
 ...
 *
 */
double	ceilY			= 0.12;	// upper point in y direction
double	inletFloorY		= 0.0;	// lower point in y direction at the inlet
...
double	tresholdX		= 0.1;// treshold position in x direction
double	outletX			= 0.1;// outlet in x direction
...
double	k				= 500;	//500 for water linear spring constant of the contact
double	c				= 0.07;//0.07 for water 2.0*sqrt(maxMass*k);		// damping constant for the spring
double	vMean			= 0.05;	// mean velocity in m/s
double	angle			= 0;		// angle in which gravity works in degrees
CPair	gravity			= CPair(0.0,-GRAVITY);
double	swarmImpact		= 0.0;//0.0007;		// determines how big the influence of the swarm is
double friction			= 1.0;			// % of vel that remains after collision
...
const int showTime		= 30;		// draw every 10 calculations
...
// -----------------------------------
CContainer container(maxNrBalls, maxNrWalls, maxNrObjects, minRadius, maxRadius, minMass, maxMass);
