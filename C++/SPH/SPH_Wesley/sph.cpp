...
// for output
std::ofstream outputFile("output.txt", ios::out);
...
sod(xx, vx, yy, vy, zz, vz, mass, den, p, u, hh, gamma, xmax, xmin,
...
float *swap_p = new float[n],  *swap_den = new float[n], *swap_hh = new float[n], 
	  *swap_mass = new float[n], *swap_u = new float[n], *swap_au = new float[n];
for(i = 0; i < n; i++) {
...
while(t <= tend) {
	...
	*/
    ...
    // bouyancy(xx, yy, zz, az, den, n, b1, grav); // FIXME
	...
	*/
	 ...
	 */
      ...
      if ((t>twr1)&&(t<=(twr1+delt))) {
         for(i=0; i<n; ++i) {
			outputFile<<xx[i]<<" "<<den[i]<<" "<<p[i]<<" "<<vx[i]<<" "<<u[i]<<" "<<hh[i]<<endl;
         }
	  }

      if ((t>twr2)&&(t<=(twr2+delt))) {
         for(i=0; i<n; ++i) {
            outputFile<<xx[i]<<" "<<den[i]<<" "<<p[i]<<" "<<vx[i]<<" "<<u[i]<<" "<<hh[i]<<endl;
         }
      }

      if ((t>twr3)&&(t<=(twr3+delt))) {
         for(i=0; i<n; ++i) {
            outputFile<<xx[i]<<" "<<den[i]<<" "<<p[i]<<" "<<vx[i]<<" "<<u[i]<<" "<<hh[i]<<endl;
         }
      }

      if ((t>twr4)&&(t<=(twr4+delt))) {
         for(i=0; i<n; ++i) {
            outputFile<<xx[i]<<" "<<den[i]<<" "<<p[i]<<" "<<vx[i]<<" "<<u[i]<<" "<<hh[i]<<endl;
         }
      }

      if ((t>twr5)&&(t<=(twr5+delt))) {
         for(i=0; i<n; ++i) {
            outputFile<<xx[i]<<" "<<den[i]<<" "<<p[i]<<" "<<vx[i]<<" "<<u[i]<<" "<<hh[i]<<endl;
         }
      }

      if ((t>twr6)&&(t<=(twr6+delt))) {
         for(i=0; i<n; ++i) {
            outputFile<<xx[i]<<" "<<den[i]<<" "<<p[i]<<" "<<vx[i]<<" "<<u[i]<<" "<<hh[i]<<endl;
         }
      }

      if ((t>twr7)&&(t<=(twr7+delt))) {
         for(i=0; i<n; ++i) {
            outputFile<<xx[i]<<" "<<den[i]<<" "<<p[i]<<" "<<vx[i]<<" "<<u[i]<<" "<<hh[i]<<endl;
         }
      }

      if ((t>twr8)&&(t<=(twr8+delt))) {
         for(i=0; i<n; ++i) {
            outputFile<<xx[i]<<" "<<den[i]<<" "<<p[i]<<" "<<vx[i]<<" "<<u[i]<<" "<<hh[i]<<endl;
         }
      }
