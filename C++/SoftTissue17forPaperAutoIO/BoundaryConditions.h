...
public:
	BoundaryConditions(){}
	~BoundaryConditions(){}
	void		initialize( Corner* cornerA, Spring* springA, Spring* secSpringA, const char* nameIn, const char* nameParam );
	double		getLength(){ return getEastPos() - getWestPos(); }
	double		getWidth(){ return 100.0*(initialWidth-(northPoint->getPos().y-southPoint->getPos().y))/initialWidth;}
...
// ----------------------
void BoundaryConditions::initialize(Corner* cornerA, Spring* springA, Spring* secSpringA, const char* nameIn, const char* nameParam)
{
	{ // read from inputFile.txt
//auto//		const char* nameIn = "data/inputFile.txt";
		InputFile iF(nameIn);
		...
		initialWidth = northPoint->getPos().y-southPoint->getPos().y;
	/*	glColor3f(0,1,0);
...
// apply the BC
// ------------
bool BoundaryConditions::apply(int mode)
{
	if((strain<0        )&&(stretching==-1)){stretching *= -1; /*exit(0);*/}
	if((strain>maxStrain)&&(stretching==+1)){stretching *= -1; return false;}
	int i;
	if(mode==1){ // moving
		...
		for(i=0; i<nrEastCorners; ++i)
		{
			...
			pEastCorner[i]->setVel(ZEROPAIR);
			pEastCorner[i]->setAcc(ZEROPAIR);
		}
		westPosX -= stretching*stretchVel*dt;
		for(i=0; i<nrWestCorners; ++i)
		{
			pWestCorner[i]->setPosX(westPosX);
			pWestCorner[i]->setVel(ZEROPAIR);
			pWestCorner[i]->setAcc(ZEROPAIR);
		}
