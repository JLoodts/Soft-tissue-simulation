...
{	
public:	
	Container(const char* nameInNew, const char* nameOutNew, const char* nameParamNew );
	~Container(){};
	void	draw();
	void	move( double dt );
	void	calculateForces();
	bool	applyBC(int mode){ return BC.apply(mode); }
	void	save();
...
// -----------------------
Container::Container( const char* nameInNew, const char* nameOutNew, const char* nameParamNew )
{
	nameIn = nameInNew;
	nameOut = nameOutNew;
	nameParam = nameParamNew; 
//auto//	const char* nameOut = "data/forceDeformation.txt";
	ofstream outFile;
...
//auto//	const char* nameIn = "data/inputFile.txt";
	...
	for(i=0; i<nrCells; ++i)
	{
		cellA[i].initialize(i+1, nrCornersA[i], cornerNrsA[i], cornerA,
							     nrSpringsA[i], springNrsA[i], springA, nameParam);
	}
	delete[] nrCornersA;
	delete[] cornerNrsA;
	delete[] nrSpringsA;
	...
	Pair max = cornerA[nrCorners-1].getPos();
	Pair min = cornerA[0].getPos();
	Pair pos = max - min;
	if(pos.x<pos.y){
	...
	// initialize the BC
	BC.initialize(cornerA, springA, secSpringA, nameIn, nameParam);
...
/*	for( i=0; i<nrSprings; ++i ){ springA[i].draw(); }
	...
	glColor3f(0,1,0);
	drawDisc(cornerA[northNr-1].getPos(), 0.0001); 
	glColor3f(0,1,1);
	drawDisc(cornerA[southNr-1].getPos(), 0.0001); 
*/
}
