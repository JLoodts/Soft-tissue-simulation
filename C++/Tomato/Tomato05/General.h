...
{
	return sin(2.0*PI*angle/360.0);
}

double cosDeg(double angle)
{
	return cos(2.0*PI*angle/360.0);
}
...
public:
	...
	CTriple  operator- ()	const
	{ // unary minus
		return CTriple(-x, -y, -z);			}
...
{
public:
	int		x,	// x-value of the CTriple
			y,  // y-value of the CTriple
			z;	// z-value of the CTriple
	CTripleInt(){}
	CTripleInt(int newx, int newy, int newz)
			:x(newx),   y(newy),   z(newz){}

	CTripleInt  operator- ()	const
	{ // unary minus
		return CTripleInt(-x, -y, -z);			}
	CTripleInt operator+ (CTripleInt &B)
	{ // sum of two vectors
		return CTripleInt(x + B.x, y + B.y, z + B.z); }
	CTripleInt operator- (CTripleInt &B)
	{ // difference of two vectors	
		return CTripleInt(x - B.x, y - B.y, z - B.z); }
	double operator* (CTripleInt &B)
	{ // dot product between two vectors
		return x * B.x + y * B.y + z * B.z;}
	CTripleInt operator* (int b)
	{ // vector multiplied by a scalar
		return CTripleInt(b * x, b * y, b * z);}
	friend CTripleInt operator* (int b, CTripleInt &A)
	{ // scalar multiplied by a vector
		return CTripleInt(b * A.x, b * A.y, b * A.z);	}
};														

#define ZEROTRIPLE CTriple(0,0,0)
#define ZEROTRIPLEINT CTripleInt(0,0,0)


CTriple Bezier(CTriple P0, CTriple P1, CTriple P2, CTriple P3, float t)
{// 0<=t<=1
...
public:
	int	x,
		y;
	CPairInt(){}
	CPairInt(int newx, int newy)
		 :x(newx), y(newy){}
	CPairInt(const CPairInt & newP)
		: x(newP.x), y(newP.y){}
};
