...
//		stretchVel = 0.5*(1.0/(1.0+(exp(-x))));
		northPosY += stretching*stretchVel*dt;
		southPosY -= stretching*stretchVel*dt;
...
void BoundaryConditions::draw()
{
	glColor4f(0,0,1,0.4);
	double width = 0.004;
	drawPlane(CTriple(0,0,width),CTriple(width,0,0), CTriple(southPoint->getPos().x-0.5*width,southPosY-0.00002,southPoint->getPos().z-0.5*width));
	drawPlane(CTriple(0,0,width),CTriple(width,0,0), CTriple(northPoint->getPos().x-0.5*width,northPosY+0.00002,northPoint->getPos().z-0.5*width));
}
