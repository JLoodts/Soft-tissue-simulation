...
public:	
	...
	void	freeze				(											);
	void	pullUp();
	double	getVolume(){ cellA[42].drawFull(); return cellA[42].calculateSurface();}
	double	getNorthPos();
	double	getSouthPos();
...
CContainer::CContainer( double newCornerMass, double newPressure )
{
	...
	double scalingFactor = 1;//0.00428;
 ...
 */
{
	...
	for(i=0; i<nrCells; ++i)
	{
		if(i>=0) {mode=1;// also pressure and secondary springs
		} else {mode=2;} // only the springs of the walls
		cellA[i].calculateF(k,c,mode );
	}
